...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener {
    ...
    public void move() {
        ...
        // Remove used/out-of-bounds bullets
        bulletArray.removeIf(b -> b.used || b.y < 0);
        alienBullets.removeIf(b -> b.y > boardHeight);

        Random rand = new Random();
        if (rand.nextInt(100) < 1) {
            ArrayList<Block> shooters = new ArrayList<>();
            for (Block alien : alienArray) {
                if (alien.alive)
                    shooters.add(alien);
            }
            if (!shooters.isEmpty()) {
                Block shooter = shooters.get(rand.nextInt(shooters.size()));
                Block alienBullet = new Block(shooter.x + shooter.width / 2, shooter.y + shooter.height, bulletWidth,
                        bulletHeight, null);
